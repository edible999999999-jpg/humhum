...
`client_registry.rs` 定义了 5 个客户端的 profile（Claude Code、Codex、Qwen Code、Gemini CLI、Kimi K1），每个包含配置格式（JSON/TOML）、配置路径、支持的 hook 事件。Settings 面板 Hooks tab 可独立安装/卸载各客户端的 hooks。
