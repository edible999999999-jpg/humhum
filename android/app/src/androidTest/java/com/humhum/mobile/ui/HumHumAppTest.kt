...
class HumHumAppTest {
    ...
    fun humiFirstViewportKeepsFourRoomsAndSettingsSeparate() {
        ...
        compose.onAllNodesWithTag("role-navigation-icon", useUnmergedTree = true)
            .assertCountEquals(4)
        compose.onNodeWithTag("role-symbol-mic-2", useUnmergedTree = true)
            .assertIsDisplayed()
        compose.onNodeWithTag("role-symbol-radio-tower", useUnmergedTree = true)
            .assertIsDisplayed()
        compose.onNodeWithTag("role-symbol-eye", useUnmergedTree = true)
            .assertIsDisplayed()
        compose.onNodeWithTag("role-symbol-wrench", useUnmergedTree = true)
            .assertIsDisplayed()
