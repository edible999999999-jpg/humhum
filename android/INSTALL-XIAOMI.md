...
1. Download `HUMHUM-Android-0.3.17-Xiaomi.zip`.
2. Open Xiaomi File Manager and extract the ZIP into the local `Download` folder.
3. Open `HUMHUM-Android-0.3.17.apk` from the extracted folder. Do not launch the APK directly from the browser or WeChat download notification; some Xiaomi download providers lose the temporary file path and report `open failed: ENOENT`.
