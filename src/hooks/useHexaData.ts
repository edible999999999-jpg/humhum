...
import { buildSupervisorSession, watchedOnlySession } from "./hexaSupervisorModel";

export { agentLabel } from "./hexaSupervisorModel";
...
// Receipt returned by every agent send/retry (codex, claude, opencode alike),
// not codex-specific despite the historical name.
...
export function useHexaData() {
  ...
  const sendCodexMessage = useCallback(async (threadId: string, message: string) => {
    try {
      return await invoke<AgentSendReceipt>("hexa_send_codex_message", { threadId, message });
    } finally {
      await fetchSessions();
    }
  }, [fetchSessions]);
  ...
  const retryCodexMessage = useCallback(async (interventionId: string) => {
    try {
      return await invoke<AgentSendReceipt>("hexa_retry_codex_message", { interventionId });
    } finally {
      await fetchSessions();
    }
  }, [fetchSessions]);
  ...
  const sendClaudeMessage = useCallback(async (sessionId: string, message: string) => {
    try {
      return await invoke<AgentSendReceipt>("hexa_send_claude_message", { sessionId, message });
    } finally {
      await fetchSessions();
    }
  }, [fetchSessions]);
  ...
  const retryClaudeMessage = useCallback(async (interventionId: string) => {
    try {
      return await invoke<AgentSendReceipt>("hexa_retry_claude_message", { interventionId });
    } finally {
      await fetchSessions();
    }
  }, [fetchSessions]);
  ...
  const sendOpenCodeMessage = useCallback(async (sessionId: string, message: string) => {
    try {
      return await invoke<AgentSendReceipt>("hexa_send_opencode_message", { sessionId, message });
    } finally {
      await fetchSessions();
    }
  }, [fetchSessions]);
  ...
  const retryOpenCodeMessage = useCallback(async (interventionId: string) => {
    try {
      return await invoke<AgentSendReceipt>("hexa_retry_opencode_message", { interventionId });
    } finally {
      await fetchSessions();
    }
  }, [fetchSessions]);
  ...
  const configureMobileRelay = useCallback(async (
    ...
    if (mobileBridge.enabled) {
      throw new Error(t("hexa.relayCloseFirst"));
    }
