...
- [ ] **Step 1: Write failing policy tests** for case-insensitive Xiaomi/Redmi/Poco/BlackShark recognition, generic-device rejection, factual exempt/restricted status text, ordered allow-listed MIUI/HyperOS targets, and an empty generic target list.
- [ ] **Step 2: Run** `./gradlew :app:testDebugUnitTest --tests com.humhum.mobile.DeviceCarePlanTest` and require compilation failure because `DeviceCarePlan` does not exist.
- [ ] **Step 3: Implement the minimal immutable policy** with exact target constants and no Android framework dependency.
- [ ] **Step 4: Run the focused test** and require all policy cases to pass.
- [ ] **Step 5: Extend the manifest contract test** to assert the exact existing permission set still excludes `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`, then run it green.
- [ ] **Step 6: Implement navigation** using `Intent(Settings.ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS)`, resolved explicit Xiaomi components, and `Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS, Uri.parse("package:" + activity.getPackageName()))` fallback. Catch only launch-time runtime failures and try the next safe target.
- [ ] **Step 7: Commit** `feat(android): add Xiaomi background settings assistant`.
...
- [ ] **Step 1: Add stable layout controls** beneath the monitor switch: one status line and two 42dp command buttons; keep the autostart button `gone` unless `DeviceCarePlan.isXiaomiFamily(Build.MANUFACTURER)`.
- [ ] **Step 2: Bind click handlers** to the navigator and expose a short error only if every safe settings target fails.
- [ ] **Step 3: Refresh factual battery state in `onResume`** using `PowerManager.isIgnoringBatteryOptimizations(getPackageName())`; never infer Xiaomi autostart state.
- [ ] **Step 4: Run Android unit tests and `lintDebug`** and require no layout, accessibility, or permission regression.
- [ ] **Step 5: Commit** `feat(android): expose background reliability controls`.
...
- [ ] **Step 1: Write failing gate tests** proving initial availability schedules once, duplicate availability coalesces, loss rearms recovery, and a second availability schedules once.
- [ ] **Step 2: Run the focused test** and require compilation failure because `NetworkRecoveryGate` does not exist.
- [ ] **Step 3: Implement the minimal synchronized gate** and run the focused test green.
- [ ] **Step 4: Register one `ConnectivityManager.NetworkCallback` in service `onCreate`**. Route `onAvailable` through the existing single-thread scheduler and call `schedule(0)` only when the gate returns true; route `onLost` to the gate.
- [ ] **Step 5: Unregister exactly once in `onDestroy`**. Treat registration/unregistration runtime errors as nonfatal because bounded polling remains the fallback.
- [ ] **Step 6: Run all Android tests and release lint/build**.
- [ ] **Step 7: Commit** `feat(android): recover monitor when network returns`.
...
- [ ] **Step 1: Install and cold-launch the signed APK** on API 36 ARM64; capture a screenshot proving controls fit without overlap.
- [ ] **Step 2: Use UI automation to open standard battery optimization settings** and return to HUMHUM; verify no new runtime permission prompt appears.
- [ ] **Step 3: Enable monitoring, force a long retry state, toggle Wi-Fi off/on, and verify a Bridge request occurs promptly after network return while foreground service type remains `0x200`**.
- [ ] **Step 4: Reboot the emulator** and verify explicit-enabled monitor restoration still works; revoke the disposable device and verify cleanup.
- [ ] **Step 5: Run** Android tests/lint/release build, frontend tests/build, Rust tests, `apksigner --Werr`, `jarsigner -verify`, manifest inspection, and desktop HTTPS root smoke.
- [ ] **Step 6: Copy only verified APK/AAB, record new hashes, update docs, confirm paired-device count zero, and stop the emulator.**
- [ ] **Step 7: Commit** `docs(android): verify Xiaomi survival assistant`.
