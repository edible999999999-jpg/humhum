...
| Broad client coverage | Partial | Managed profiles include Claude Code, Codex, Qwen Code, Gemini CLI, Kimi, QoderWork, Qoder, CodeBuddy, WorkBuddy, Cursor, GitHub Copilot CLI, OpenCode and Hermes Agent; local Pi and Wukong watchers also exist. Hermes lifecycle/progress delivery is protocol-smoke verified through its official plugin hooks, while Copilot, Cursor 3.10.20 and OpenCode 1.17.18 have installed runtime evidence. OpenClaw, Hermes follow-ups and remote variants remain incomplete. |
...
- Local release artifact: `src-tauri/target/release/bundle/dmg/HumHum_0.1.0_aarch64.dmg` (43 MB), SHA-256 `98bb252b4448fd312c17eeef7dc57421f1e633fb86457bc7b874f2710a954848`. It includes Hermes Agent supervision and identity alongside provider-aware mobile follow-ups, OpenCode completion reliability, Ghostty terminal-ID routing, privacy-bounded session change summaries, permissions, sounds, themes, and wake guard. The DMG passed `hdiutil verify`; its read-only mounted app passed strict deep ad-hoc signature verification and contains a thin arm64 executable. It is not Developer ID signed or notarized, so it is not yet a frictionless public download.
- Rust: 111 passed, 2 ignored. Frontend: 25 passed across 13 files. Production frontend build: passed.

## Next Iteration Order

1. Add OpenClaw supervision and exact IDE chat routing where the host exposes a stable conversation command.
