...
- Local release artifact: `src-tauri/target/release/bundle/dmg/HumHum_0.1.0_aarch64.dmg` (43 MB), SHA-256 `2c360548c8950fcc29830ed11c3df596ee3f9671fb6c27a347038729c53ffbe3`. It includes provider-aware mobile follow-ups, OpenCode completion reliability, Ghostty terminal-ID routing, and privacy-bounded session change summaries alongside the earlier permissions, sounds, themes, and wake guard. The DMG passed `hdiutil verify`; its read-only mounted app passed strict deep ad-hoc signature verification and contains a thin arm64 executable. It is not Developer ID signed or notarized, so it is not yet a frictionless public download.
