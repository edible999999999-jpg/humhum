...
<p align="center">
  <a href="https://github.com/edible999999-jpg/humhum/releases/download/v0.3.17-beta.2/HumHum_0.3.17_aarch64.dmg"><strong>下载 macOS 版</strong></a>
  ·
  ...
  ·
  <a href="https://github.com/edible999999-jpg/humhum/releases/download/v0.3.17-beta.2/HUMHUM-Android-0.3.17.apk"><strong>下载 Android APK</strong></a>
  ·
...
HUMHUM 已内置自己的无网络、只读原生读取器，用于读取本机微信 4.x 最近的
私聊与群聊。它只通过 stdin 接收一次结构化请求，仅允许 `status`、`sessions`
和 `timeline` 三个动作，没有发送消息、远程 companion、更新器、Shell 或导出
路径；每次运行前还会校验读取器和 WCDB 组件的 SHA-256。

最小 WCDB 兼容层来自对
[`r266-tech/wechat-cli`](https://github.com/r266-tech/wechat-cli)
固定版本的审计与独立裁剪，准确 commit 和许可证记录在
...
- `hush-inbox.json` —— Hush 的本地消息收件箱（最多保留 2,000 条）
- `hush/structured-signals.sqlite3` —— 来自已授权手机的加密健康日汇总
